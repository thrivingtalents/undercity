...
def write_json(path, payload):
    """UTF-8 and LF on every platform. kit/MANIFEST.json fingerprints these
    files, so a CRLF copy of identical content reads as drift, and the
    server refuses to say the printed kit matches what it is running."""
...
write_json(OUTDIR / "faults.json", {"meta": meta, "faults": faults})
write_json(OUTDIR / "specs.json", {"meta": meta, "specs": list(specs.values())})
write_json(OUTDIR / "sectors.json", {"meta": meta, "sectors": sectors})
