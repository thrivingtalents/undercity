...
function broadcastFor(game, { editable = false, full = true } = {}) {
  ...
  return {
    ...
    focus: game.focusView ? game.focusView() : null,
    statuses: GameStateStatuses(game),
    editable,
  };
}

/** The four words COMM may report. Sent so the console never hardcodes them. */
function GameStateStatuses(game) {
  const ctor = game && game.constructor;
  return (ctor && ctor.REPORTED_STATUSES) || ['STABLE', 'DEGRADED', 'CRITICAL', 'DARK'];
}
