...
function forSector(game, sectorCode) {
  ...
  const extra = {
    ...
    // The rules this screen must honour: whether we may accept, whether
    // Transport needs the paper chit, whether an inbound request rings.
    transfer_rules: {
  ...
  if (sectorCode === 'TRN') {
    ...
    const needsAccept = game.cfg.require_supplier_acceptance !== false;
    const capacity = game.trnCapacity();
    const used = game.stampsUsed();
    const open = game.state.transfers.filter((t) => ['REQUESTED', 'AGREED', 'WAITING_TRN'].includes(t.status));
    // Transport never sees a request the supplier has not accepted: until then
    // there is nothing to carry and nothing to stamp.
    const visible = needsAccept ? open.filter((t) => t.status !== 'REQUESTED') : open;
    extra.transfer_queue = {
      ...
      awaiting_acceptance: open.length - visible.length,
      items: visible.map((t) => ({
        ...transferView(t),
