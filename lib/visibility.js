...
function roundOutputView(game, s) {
  ...
  const used = !!(s.round_output && s.round_output.round === game.state.round);
...
/** NEXT ROUND UPKEEP readiness, from the sector's real stock and nothing else. */
...
function movementFor(game, code) {
  ...
  for (const r of game.state.requests) {
    ...
    const incoming = r.supplier === code;                 // another table asked US
    const active = r.status === 'REQUESTED';              // once fulfilled, the transfer is the card
    const canFulfill = active && incoming && (game.cfg.enforce_supplier_stock === false
      || game.supplierAvailable({ from: code, resource: r.resource }) >= r.amount);
    cards.push({
      kind: 'request', id: r.id, from: r.supplier, to: r.requester, resource: r.resource, amount: r.amount,
      status: r.status,
      label: active && incoming ? 'ACTION REQUIRED' : (REQUEST_LABEL[r.status] || r.status),
      direction: incoming ? 'INCOMING' : 'OUTGOING',
      active,
      action_required: active && incoming,
      can_fulfill: canFulfill,                            // our own stock only — never another table's
      can_withdraw: active && r.requester === code,
      linked_id: r.transfer_id || null,
      at: r.requested_at, updated_at: r.updated_at || r.requested_at,
    });
  }
  for (const t of game.state.transfers) {
    ...
    cards.push({
      kind: 'transfer', id: t.id, from: t.from, to: t.to, resource: t.resource, amount: t.amount,
      status: t.status,
      label: TRANSFER_LABEL[t.status] || t.status,
      direction: t.from === code ? 'OUTGOING' : 'INCOMING',
      active,
      action_required: false, can_fulfill: false, can_withdraw: false,
      linked_id: t.request_id || null,
      at: t.created_at || t.requested_at, updated_at: t.updated_at || t.created_at || t.requested_at,
    });
  }
  const ts = (x) => Date.parse(x) || 0;
  const active = cards.filter((c) => c.active).sort((a, b) => (ts(a.at) - ts(b.at)) || a.id.localeCompare(b.id));
  const closed = cards.filter((c) => !c.active).sort((a, b) => (ts(b.updated_at) - ts(a.updated_at)) || b.id.localeCompare(a.id));
...
function ownSector(game, s, showDependency) {
  return {
    ...
    // Upkeep falls due when the round ends: the round clock is the upkeep clock.
    upkeep_due_in_s: Math.ceil(game.state.round_clock.remaining_s),
