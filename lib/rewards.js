...
function describeEffects(p) {
  ...
  for (const c of p.capacity_effects || []) bits.push(c.system === 'TRN_APPROVALS' ? `TRN +${c.amount} APPROVAL THIS ROUND` : `MED +${c.amount} HEAL THIS ROUND`);
...
function resultLine(reward, { health = {}, resources = {}, targets = [], effects = [] } = {}) {
  ...
  for (const c of reward.capacity_effects || []) bits.push(c.system === 'TRN_APPROVALS' ? 'TRN +1 APPROVAL THIS ROUND' : 'MED +1 HEAL THIS ROUND');
