...
function submitCode(game, { sector: sectorCode, fault_code: faultCode, code, workers_assigned }) {
  ...
  try {
    ...
    if (deduct) {
      ...
      consumed = {};
      if (Object.keys(short).length) {
        game.log.write('materials_unverified', { sector: sectorCode, fault: faultCode, short, required: { ...fault.resources_required } });
      } else {
        for (const [k, v] of Object.entries(fault.resources_required || {})) {
    ...
    game.log.write('repair_completed', {
      ...
      reward_instance_id: fault.reward ? fault.reward.reward_instance_id : null,
      exact_reward: fault.reward ? fault.reward.display_label : null,
