...
function submitCode(game, { sector: sectorCode, fault_code: faultCode, code, workers_assigned }) {
  ...
  // 3. Crew. The requirement stays in the binder: a refusal says only that
  ...
  if (cfg.auto_economy && cfg.resolve_requires_resources) {
    ...
    if (Object.keys(short).length) {
      // Which materials, and how many, is the binder's to tell.
  ...
  // 6. The fault's reward, once per run, paid to the owner — the only place a
  //    player action earns one. Resolution, reward, claim and log land together.
  ...
  game.log.write('submit', {
    ...
    reward: reward.applied ? { resources: reward.resources, health: reward.health, key: reward.key } : { applied: false, reason: reward.reason },
  });
  game.sting('resolved');
  game.touch();
