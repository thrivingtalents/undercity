...
class RunLog {
  ...
  write(ev, fields = {}) {
    ...
    // An event that stamps a period of its own (COM's board, AGR's hand, all
    // stamped with the operating cycle since 2026-09-21) would otherwise bury
    // the backend round it happened in. Analytics folds by that, so keep it.
    if (fields.round !== undefined && this.context.round !== undefined && fields.round !== this.context.round) {
      entry.context_round = this.context.round;
    }
    try {
