...
const FAULT_REWARDS = require('./fault-rewards.json');
...
class GameState {
  ...
  clearFault(sectorCode, faultCode, reason) {
    ...
    fault.reward = this.applyFaultReward(sectorCode, fault, { via, by: 'facilitator' });
    this.touch();
    return true;
  }
  ...
  // -- fault rewards ------------------------------------------------------------
  //
  // Every fault carries one reward (lib/fault-rewards.json), paid to the sector
  // that owns it, once per run, and only by the authoritative completion —
  // an accepted code, or the facilitator clearing a ghost. Opening, viewing,
  // assigning crew, a wrong code, a failed attempt: nothing. A reward never
  // approves a transfer, heals a worker, touches COM's board or plays a card.

  /** The reward for a fault code, after any scenario override. Null when none. */
  faultReward(code) {
    const base = FAULT_REWARDS.rewards[code];
    const over = (this.cfg.fault_reward_overrides || {})[code];
    if (!base && !over) return null;
    const merged = { ...(base || {}), ...(over || {}) };
    const resources = {};
    for (const [k, v] of Object.entries(merged.resources || {})) {
      const n = Math.max(0, Math.floor(Number(v) || 0));
      if (n > 0) resources[k] = n;
    }
    return {
      owner: merged.owner || null,
      phase: merged.phase || FAULT_REWARDS.phase_of_round[(this.faultsByCode.get(code) || {}).round] || null,
      type: merged.type || (Object.keys(resources).length && merged.health ? 'MIXED' : merged.health ? 'HEALTH' : 'RESOURCE'),
      resources,
      health: Math.max(0, Math.floor(Number(merged.health) || 0)),
      rvu: Number(merged.rvu) || 0,
    };
  }

  /** "+2 POWER · +5 SECTOR HEALTH" — the words a player sees. Never the units. */
  rewardText(reward) {
    if (!reward) return '';
    const names = { power: 'POWER', water: 'WATER', parts: 'PARTS', med: 'MEDICAL', workers: 'WORKERS' };
    const bits = Object.entries(reward.resources || {}).map(([k, v]) => `+${v} ${names[k] || String(k).toUpperCase()}`);
    if (reward.health > 0) bits.push(`+${reward.health} SECTOR HEALTH`);
    return bits.join(' · ');
  }

  /** What a screen may show before completion, or null when the preview is off. */
  rewardPreview(code) {
    if (this.cfg.fault_rewards_enabled === false || this.cfg.show_fault_reward_preview === false) return null;
    const r = this.faultReward(code);
    if (!r) return null;
    return { resources: { ...r.resources }, health: r.health, text: this.rewardText(r) };
  }

  /**
   * Pay a fault's reward, once. Called only from the completion path.
   *   via 'resolve'            an accepted code — always authoritative
   *   via 'false_alarm_clear'  the facilitator clearing a ghost — authoritative
   *   via 'force'              the facilitator clearing a real fault — no reward
   *                            unless reward_on_facilitator_force_resolve is on
   * Health is capped, and a sector at the floor stays there: a reward never
   * revives a DARK district.
   */
  applyFaultReward(sectorCode, fault, { via = 'resolve', by = 'sector' } = {}) {
    const code = fault.code;
    const key = `faultReward:${this.state.run_id}:${code}`;
    const common = { fault: code, sector: sectorCode, key, via, by, run_id: this.state.run_id, phase: null };
    if (this.cfg.fault_rewards_enabled === false) return { applied: false, reason: 'disabled', key };
    const reward = this.faultReward(code);
    if (!reward) return { applied: false, reason: 'no_reward', key };
    common.phase = reward.phase;

    if (this.state.rewards_claimed[code]) {
      this.log.write('fault_reward_duplicate_blocked', { ...common, first_claim: this.state.rewards_claimed[code].at });
      return { applied: false, reason: 'duplicate', key, text: this.rewardText(reward) };
    }
    const override = via === 'force';
    if (override && !this.cfg.reward_on_facilitator_force_resolve) {
      this.log.write('fault_reward_force_resolve_skipped', { ...common, rvu: reward.rvu });
      return { applied: false, reason: 'force_resolve', key, text: this.rewardText(reward) };
    }

    const sector = this.state.sectors[sectorCode];
    if (!sector) return { applied: false, reason: 'unknown_sector', key };
    const cap = Number(this.cfg.reward_health_cap ?? 100);
    const healthBefore = Math.round(sector.integrity * 10) / 10;
    const inventoryBefore = { ...sector.inventory };

    // Resources: straight into the owner's real inventory. Nobody else's, and
    // never COM's board — that stays whatever COM last reported.
    for (const [k, v] of Object.entries(reward.resources)) {
      sector.inventory[k] = (Number(sector.inventory[k]) || 0) + v;
    }
    // Health: percentage points, capped, overflow discarded; the floor is final.
    let healthApplied = 0;
    if (reward.health > 0 && sector.integrity > 0 && sector.status !== 'DARK') {
      const next = Math.min(cap, sector.integrity + reward.health);
      healthApplied = Math.round((next - sector.integrity) * 10) / 10;
      sector.integrity = next;
      this.refreshStatus(sector);
    }

    const claim = {
      key, at: new Date().toISOString(), sector: sectorCode, via, by, instance: fault.id || null,
      resources: { ...reward.resources }, health: healthApplied, rvu: reward.rvu,
    };
    this.state.rewards_claimed[code] = claim;
    const healthAfter = Math.round(sector.integrity * 10) / 10;
    this.log.write('fault_reward_applied', {
      ...common, reward_type: reward.type, resources_added: { ...reward.resources },
      health_requested: reward.health, health_before: healthBefore, health_after: healthAfter,
      inventory_before: inventoryBefore, inventory_after: { ...sector.inventory },
      rvu: reward.rvu, reward_claimed: true, facilitator_override: override,
    });
    if (override) this.log.write('fault_reward_admin_override', { ...common, rvu: reward.rvu });
    this.touch();
    return {
      applied: true, key, via, resources: { ...reward.resources }, health: healthApplied,
      health_before: healthBefore, health_after: healthAfter, text: this.rewardText(reward),
