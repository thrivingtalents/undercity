...
class GameState {
  ...
  /**
   * Per-fault tuning (spec §44): a deadline, an expiry penalty and EXTRA
  ...
  setFaultOverride(faultCode, patch) {
    ...
    if (patch === null || patch === undefined) {
      ...
      const clean = {};
      if ('deadline_s' in patch) clean.deadline_s = patch.deadline_s === null || patch.deadline_s === '' ? null : Math.max(0, Number(patch.deadline_s) || 0);
      if ('integrity_penalty' in patch && patch.integrity_penalty !== null && patch.integrity_penalty !== '') {
        clean.integrity_penalty = Math.max(0, Number(patch.integrity_penalty) || 0);
      }
  ...
  fireFault(faultCode, sectorCode, { source = 'facilitator' } = {}) {
    ...
    // Deadline: the scenario's per-fault override, else the content's own,
    // else the scenario default for the severity.
    const override = (this.cfg.fault_overrides || {})[def.code] || {};
    let deadline = 'deadline_s' in override ? override.deadline_s : def.deadline_s;
    if (deadline == null) {
    ...
    const penalty = override.integrity_penalty != null
      ? Number(override.integrity_penalty)
      : Number((this.cfg.deadline_penalty || {})[String(def.severity)] ?? 0);
  ...
  /**
   * Transport's stamp allowance for the current period, after brownout, DARK
   * and temporary effects. The PERIOD is `transfer_limit_basis`: 'round' (the
   * default — three stamps for the whole of R2, not three every cycle) or
   * 'cycle' (the original behaviour, kept as configuration).
   */
  trnCapacity() {
    let cap = Number(this.cfg.transport_stamp_limit ?? this.cfg.trn_capacity_per_cycle ?? 3);
  ...
  /** Which counter the allowance is measured against. */
  stampBasis() {
    return this.cfg.transfer_limit_basis === 'cycle' ? 'cycle' : 'round';
  }

  /**
   * Stamps used in the current period. BOTH counters are always maintained, so
   * the facilitator can switch the basis mid-run and get a sane number either
   * way; only the active one gates the stamp.
   */
  stampsUsed() {
    return this.stampBasis() === 'cycle'
      ? Number(this.state.cycle.stamped || 0)
      : Number(this.state.stamps_this_round || 0);
  }

  /** Older name for the active counter, kept for callers outside this file. */
  transfersStampedThisCycle() {
    return this.stampsUsed();
  }

  /**
   * Zero the allowance. `which` names the counter: 'round' at a round change,
   * 'cycle' at a cycle boundary, 'all' for the facilitator's manual reset.
   * A cycle boundary must never clear the round counter — that is the whole
   * point of the round basis.
   */
  resetStamps(which = 'round', { by = 'system' } = {}) {
    if (which === 'round' || which === 'all') this.state.stamps_this_round = 0;
    if (which === 'cycle' || which === 'all') this.state.cycle.stamped = 0;
    this.log.write('transport_stamp_counter_reset', { which, by, basis: this.stampBasis() });
    this.touch();
    return { ok: true, which, used: this.stampsUsed(), capacity: this.trnCapacity() };
  }

  /** What the supplier can actually hand over right now. */
  supplierAvailable(t) {
  ...
  /** Statuses that are still live: not stamped, not closed. */
  static get OPEN_TRANSFER() { return ['REQUESTED', 'AGREED', 'WAITING_TRN']; }

  /**
   * A round change is a clean sheet: anything not yet stamped is dead paper.
   * Delivered and stamped transfers are never touched, and every expiry is
   * logged so the debrief can still see what was asked for and never moved.
   */
  expirePendingTransfers({ reason = 'round_change' } = {}) {
    ...
    for (const t of this.state.transfers) {
      ...
      t.status = 'EXPIRED';
      t.expired_at = now;
      t.expire_reason = reason;
      n += 1;
      this.log.write('transfer_expired', {
        id: t.id, from: t.from, to: t.to, resource: t.resource, amount: t.amount,
        reason, round_created: t.round_created ?? null,
      });
    }
    if (n) {
      this.ticker('transfer', `${n} unstamped transfer${n === 1 ? '' : 's'} expired`, { scope: 'admin' });
      this.touch();
    }
    return n;
  }
  ...
  requestTransfer({ from, to, resource, amount, note = '', by = 'sector' }) {
    ...
    const transfer = {
      ...
      status: 'REQUESTED', requested_at: now, requested_by: by, round_created: this.state.round,
      agreed_at: null, accepted_by: null, declined_at: null, declined_by: null,
      waiting_at: null, stamped_at: null, stamped_by: null, round_stamped: null,
      delivered_at: null, cancelled_at: null, expired_at: null,
    ...
    // Scoped to the facilitator: the wall gets the completion, not every step.
    this.ticker('transfer', `Transfer requested ${from} → ${to}: ${qty} ${resource}`, { scope: 'admin' });
    this.log.write('transfer_requested', {
      id: transfer.id, from, to, resource, amount: qty, by, round_created: transfer.round_created,
    });
    this.touch();
  ...
  /**
   * SUPPLIER ACCEPT. Only the sector being asked for the resource may accept,
   * and — when stock is enforced — only if it actually holds the amount. The
   * requester can never approve its own request; that is the whole point of
   * asking. Acceptance is what exposes the transfer to Transport.
   */
  acceptTransfer(id, { by = 'facilitator' } = {}) {
    ...
    if (t.status !== 'REQUESTED') return { ok: false, reason: 'transfer_closed', status: t.status };

    const override = by === 'facilitator';
    if (!override && by !== t.from) return { ok: false, reason: 'not_supplier', supplier: t.from };

    if (this.cfg.enforce_supplier_stock !== false) {
      const have = this.supplierAvailable(t);
      if (have < t.amount) {
        this.log.write('transfer_accept_refused', {
          id, from: t.from, to: t.to, resource: t.resource, amount: t.amount,
          reason: 'insufficient_stock', have, by,
        });
        return { ok: false, reason: 'insufficient_stock_accept', have, need: t.amount, resource: t.resource };
      }
    }

    // AGREED is this engine's ACCEPTED: the name is kept so old snapshots,
    // logs and the debrief keep reading cleanly.
    t.status = 'AGREED';
    t.agreed_at = new Date().toISOString();
    t.accepted_by = by;
    if (override) t.facilitator_override = true;
    this.ticker('transfer', `${t.from} accepted ${t.to} request: ${t.amount} ${t.resource}`, { scope: 'admin' });
    this.log.write('transfer_accepted', {
  ...
  /** SUPPLIER DECLINE. Also supplier-only; the requester is told, and it ends. */
  declineTransfer(id, { by = 'facilitator', reason = null } = {}) {
    const t = this.findTransfer(id);
    if (!t) return { ok: false, reason: 'unknown_transfer' };
    if (t.status === 'EXPIRED') return { ok: false, reason: 'expired' };
    if (t.status !== 'REQUESTED') return { ok: false, reason: 'transfer_closed', status: t.status };
    ...
    t.declined_by = by;
    this.ticker('transfer', `${t.from} declined ${t.to} request: ${t.amount} ${t.resource}`, { scope: 'admin' });
   ...
   * Transport has seen it, and it is what unlocks the digital stamp.
   */
  ...
  /**
   * Move a transfer along its lifecycle. Acceptance, decline and the stamp all
   * have their own rules, so this only routes to them; STAMPED remains the one
   * step that moves stock.
   */
  updateTransfer(id, status, { by = 'facilitator' } = {}) {
    const t = this.findTransfer(id);
    if (!t) return { ok: false, reason: 'unknown_transfer' };
    if (status === 'AGREED' || status === 'ACCEPTED') return this.acceptTransfer(id, { by });
    if (status === 'DECLINED') return this.declineTransfer(id, { by });
    if (status === 'STAMPED') return this.stampTransfer(id, { by });

    if (['DELIVERED', 'CANCELLED', 'DECLINED'].includes(t.status)) return { ok: false, reason: 'transfer_closed' };
    if (t.status === 'EXPIRED') return { ok: false, reason: 'expired' };
    const now = new Date().toISOString();
    ...
    switch (status) {
      ...
      case 'CANCELLED':
        // Once the supplier has accepted, a table can no longer walk it back;
        // only the facilitator can. Before that, either party may withdraw.
        if (by !== 'facilitator' && t.status !== 'REQUESTED') {
          return { ok: false, reason: 'cancel_locked', status: t.status };
        }
  ...
  /**
   * TRANSPORT STAMP — the only step that moves stock, gated in this order:
   * supplier acceptance → the round allowance → the physical chit → stock,
   * re-checked here because it may have drained since acceptance. A refusal
   * moves nothing and consumes no allowance.
   *
   * `force` is the facilitator's override. It lifts acceptance, the allowance
   * and the chit, because those are rules; it never invents stock, because
   * that is the world. Every override is flagged in the log.
   */
  stampTransfer(id, { by = 'TRN', force = false } = {}) {
    ...
    if (!force && this.cfg.require_supplier_acceptance !== false
        && !['AGREED', 'WAITING_TRN'].includes(t.status)) {
      return refuse('not_accepted', { status: t.status, supplier: t.from });
    }
    ...
    if (auto && !partialOk && available < t.amount) {
    ...
    // Both sides move together: the checks above have already guaranteed the
    // supplier can cover it, so neither balance can be left half-applied.
    if (auto) {
    ...
    this.state.stamps_this_round = Number(this.state.stamps_this_round || 0) + 1;
    ...
    const line = `TRN stamped ${t.from} → ${t.to}: ${moved} ${t.resource}`;
  ...
  setRound(id) {
    ...
    const changed = cfg.id !== this.state.round;
    this.state.round = cfg.id;
    this.state.round_clock = { running: false, remaining_s: cfg.length_s };
    this.state.cycle.running = false;
    // A new round gives Transport its allowance back and voids every chit that
    // was never stamped. Delivered transfers are history and stay untouched.
    if (changed) {
      this.resetStamps('round', { by: 'round_change' });
      if (this.cfg.expire_pending_transfers_on_round_change !== false) {
