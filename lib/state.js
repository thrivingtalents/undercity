...
class GameState {
  ...
  migrateContinuous() {
    const LEGACY_PHASES = {
      ROUND_1: 'STABLE_OPERATIONS',
      ROUND_2: 'INTERDEPENDENCE',
      ROUND_3: 'CORE_FAILURE',
      DEBRIEF_1: 'CORE_FAILURE',
      DEBRIEF_2: 'ENDED',
      FINISHED: 'ENDED',
    };
    const known = (this.rounds.phases || []).some((p) => p.id === this.state.phase);
    if (!known) {
      const to = LEGACY_PHASES[this.state.phase] || (this.rounds.phases || [])[0].id;
      this.log.write('phase_migrated', { from: this.state.phase, to, reason: 'continuous_gameflow' });
      this.state.phase = to;
      const def = this.phaseConfig(to);
      if (def) this.state.mode = this.state.mode === 'PAUSED' ? 'PAUSED' : def.mode;
    }
  ...
  setRound(id) {
    ...
    this.ticker('round', `${cfg.id} — ${cfg.name}`, { scope: 'admin' });
    this.touch();
  }
