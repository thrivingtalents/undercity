...
class GameState {
  ...
  patchConfig(patch = {}) {
    ...
    // A new length for the live shift takes effect at once while MASTER TIME
    // has not been started; a running clock is never yanked from under the room.
    if (patch.live_length_s !== undefined && !this.state.round_clock.started && !this.state.round_clock.running) {
      this.state.round_clock.remaining_s = Math.max(0, Number(patch.live_length_s) || 0);
    }
  ...
  reset(runId, { silent = false, scenario = null } = {}) {
    ...
    for (const [code, def] of Object.entries(this.content.sectors.sectors)) {
      ...
      sectors[code] = {
        ...
        brownout_by: null,
        // What this sector chose to keep for the duration of a brownout.
        preserved_function: null,
        // TRN only: the round in which the escape valve was last used.
        emergency_movement_round: null,
        // `committed` is purpose → count: workers claimed by an action that
        // holds them for a while (GENERATOR_UPGRADE, EMERGENCY_RESTART). They
        // are still active — they are simply not free.
        workforce: { active: sc.start_workforce, injured: 0, loaned: 0, borrowed: 0, committed: {} },
        inventory: { ...sc.start_inventory },
        production: { ...sc.production },
        upkeep_per_round: { ...sc.upkeep },
        transfers_this_cycle: 0,
        // POW and WTR run a generator whose installed level sets the base they
        // produce. Null for a sector with no generator — the config decides.
        generator: null,
        faults: [],
      };
    }
    ...
    this.state = {
      ...
      paused: false,
      round: firstRound.id,
      // MASTER TIME: one clock for the whole live shift. A phase never resets it.
      round_clock: {
        running: false,
        started: false,
        remaining_s: Number(cfg.live_length_s) > 0 ? Number(cfg.live_length_s) : firstRound.length_s,
      },
      council_clock: { running: false, remaining_s: cfg.council_clock_s },
      game_clock_s: 0,
      // When the current phase's inject timeline was armed, in game seconds.
      timeline_armed_game_s: 0,
      // The facilitator's regulator: scheduled injection held, decay frozen.
      pressure: { faults_paused: false, decay_freeze_until_s: 0 },
  ...
  /**
   * A run saved before continuous gameflow (2026-09-21) carries a phase that
   * no longer exists, a round clock the length of one round, and a cycle armed
   * from the old scenario. Bring it forward without touching the city itself:
   * the phase lands on its continuous equivalent, the operating cycle picks up
   * the configured length, and a debrief phase becomes the end of the run.
   */
  migrateContinuous() {
    /*
      A run saved under an older build carries a phase id this one has never
      heard of — DEBRIEF_1 from the round era, INTERDEPENDENCE from the seven
      waves. The phases ARE the rounds now, so the round it was in IS the
      answer: match on that first and fall back to the named map only for the
      phases that ended a run. Getting this wrong would show Round 0 on a
      console whose city is three rounds deep.
    */
    const LEGACY_END = ['DEBRIEF_1', 'DEBRIEF_2', 'FINISHED', 'ENDED'];
    const phases = this.rounds.phases || [];
    const known = phases.some((p) => p.id === this.state.phase);
    if (!known) {
      const ended = LEGACY_END.includes(this.state.phase);
      const byRound = phases.find((p) => p.round === this.state.round && p.mode !== 'ENDED');
      const to = (ended ? phases.find((p) => p.mode === 'ENDED') : byRound) || byRound || phases[0];
      this.log.write('phase_migrated', { from: this.state.phase, to: to.id, round: this.state.round, reason: 'simplified_round_flow' });
      this.state.phase = to.id;
      // A run that was already playing keeps playing: only a run that had not
      // started, or one that had ended, takes the phase's own mode.
      if (this.state.mode !== 'PAUSED' && (ended || this.state.mode === 'BRIEFING')) this.state.mode = to.mode;
    }
    // One phase per round is an invariant now, so a snapshot whose pair
    // disagrees — written by a build that had several phases inside a round —
    // is repaired the same way: the round the city was actually playing wins.
    const here = this.phaseConfig();
    if (here && here.mode !== 'ENDED' && here.round !== this.state.round) {
      const fixed = phases.find((p) => p.round === this.state.round && p.mode !== 'ENDED');
      if (fixed && fixed.id !== here.id) {
        this.log.write('phase_realigned', { from: here.id, to: fixed.id, round: this.state.round });
        this.state.phase = fixed.id;
      }
    }
    // The breather is gone: a run frozen in one must not stay frozen for ever.
    delete this.state.breather;
    delete this.state.wall_debrief;
    const cycleLength = Number(this.cfg.cycle_length_s);
    if (cycleLength > 0 && this.state.cycle) {
      this.state.cycle.length_s = cycleLength;
      if (this.state.cycle.remaining_s > cycleLength) this.state.cycle.remaining_s = cycleLength;
      if (this.state.round_clock.running) this.state.cycle.running = true;
    }
    if (typeof this.state.timeline_armed_game_s !== 'number') {
      this.state.timeline_armed_game_s = this.state.game_clock_s || 0;
    }
    // COM's board was stamped with rounds. Those stamps become the cycle
    // BEFORE this one: what COM reported under the old flow is, by definition,
    // not this cycle's report, and the wall should say so rather than pass old
    // figures off as current.
    const b = this.state.broadcast;
    const previous = `C${Math.max(0, this.state.cycle.number - 1)}`;
    const restamp = (row) => {
      if (row && /^R\d+$/.test(String(row.round || ''))) row.round = previous;
    };
    if (b) {
      for (const row of Object.values(b.rows || {})) restamp(row);
      restamp(b.announcement);
    }
    if (/^R\d+$/.test(String((this.state.agr || {}).round || ''))) this.state.agr.round = previous;
    for (const s of Object.values(this.state.sectors || {})) {
      if (s.round_output && /^R\d+$/.test(String(s.round_output.round || ''))) s.round_output.round = previous;
      if (s.generator && /^R\d+$/.test(String(s.generator.upgrade_used_round || ''))) s.generator.upgrade_used_round = previous;
      if (/^R\d+$/.test(String(s.emergency_movement_round || ''))) s.emergency_movement_round = previous;
    }
  }
  ...
  restore(snapshot) {
    ...
    for (const [code, s] of Object.entries(this.state.sectors)) {
      ...
      const wf = this.state.sectors[code].workforce;
      if (!wf.committed || typeof wf.committed !== 'object') wf.committed = {};
      if (this.state.sectors[code].generator === undefined) this.state.sectors[code].generator = null;
      if (this.state.sectors[code].preserved_function === undefined) this.state.sectors[code].preserved_function = null;
      if (this.state.sectors[code].emergency_movement_round === undefined) this.state.sectors[code].emergency_movement_round = null;
    }
    ...
    if (this.state.broadcast.priority === undefined) this.state.broadcast.priority = null;
    if (this.state.broadcast.focus === undefined) this.state.broadcast.focus = null;
    for (const code of Object.keys(this.state.sectors)) {
      if (!this.state.broadcast.rows[code]) this.state.broadcast.rows[code] = { power: null, water: null, med: null, parts: null, status: null, round: null, report_round: null };
      if (this.state.broadcast.rows[code].status === undefined) this.state.broadcast.rows[code].status = null;
      // A run saved before 2026-09-24 has rows stamped with the cycle only. The
      // round it was reported in cannot be recovered from a cycle, so the card
      // shows those rows without a tag rather than guessing one.
      if (this.state.broadcast.rows[code].report_round === undefined) this.state.broadcast.rows[code].report_round = null;
    }
    ...
    this.initGenerators();
    if (!this.state.pressure || typeof this.state.pressure !== 'object') this.state.pressure = { faults_paused: false, decay_freeze_until_s: 0 };
  ...
  /**
   * Is the simulation frozen? The facilitator's emergency pause, the briefing
   * before live play, and the end of the simulation stop time. Continuous
   * gameflow (2026-09-21) removed the breather: during live play the only
   * full stop is the Admin's emergency pause.
   */
  get frozen() {
    return this.state.paused ||
      this.state.mode === 'PAUSED' || this.state.mode === 'BRIEFING' || this.state.mode === 'ENDED';
  }

  /**
   * MASTER TIME — one countdown for the whole live shift, not a round clock.
   * Phases no longer touch it; only the facilitator does (start, pause, the
   * ±1 minute steps, reset).
   */
  liveLength() {
    const configured = Number(this.cfg.live_length_s);
    if (Number.isFinite(configured) && configured > 0) return configured;
    return this.rounds.rounds
      .filter((r) => r.id !== this.rounds.rounds[0].id)
      .reduce((a, r) => a + Number(this.roundConfig(r.id).length_s || 0), 0) || 3600;
  }

  /**
   * THE PERIOD every stamped thing is measured in. Since continuous gameflow
   * that is the OPERATING CYCLE, never the round: COM's board ages by cycles,
   * AGR is dealt a cycle at a time, a producing sector generates once a cycle,
   * and a "lasts until the period ends" effect ends at the next cycle.
   */
  periodKey() { return `C${this.state.cycle.number}`; }

  /**
   * THE VISIBLE ROUND (2026-09-21). One number, 0 to 4, and the only fact
   * about the round any screen is given: R2 → 2. The rounds still carry their
   * own inject timelines and fault pools, and the facilitator's runbook still
   * names the waves inside them — on paper. Nothing names them on a screen,
   * because a descriptive title is how a room learns to predict the crisis.
   */
  roundOrdinal(id = this.state.round) {
  ...
  setStatus(code, value, { by = 'facilitator' } = {}) {
    ...
    // Leaving brownout drops the choice; entering it asks for a new one.
    if (value !== 'BROWNOUT') s.preserved_function = null;
  ...
  fireFault(faultCode, sectorCode, { source = 'facilitator' } = {}) {
    ...
    // Injection can be held without freezing the game. A facilitator firing by
    // hand still gets through — the hold is on the script, not on them.
    if (this.state.pressure && this.state.pressure.faults_paused && source !== 'facilitator') {
      return { ok: false, reason: 'fault_injection_paused', source };
    }
  ...
  injure(sectorCode, count) {
    ...
    if (moved > 0) this.reconcileCommitments(sectorCode);
  ...
  /**
   * Claim `count` free workers for `purpose`. Refuses rather than
   * over-committing, and never silently takes fewer than asked: an action
   * either has its crew or it does not start.
   */
  commitWorkers(sectorCode, purpose, count) {
    const sector = this.state.sectors[sectorCode];
    if (!sector) return { ok: false, reason: 'unknown_sector' };
    const n = Number(count) || 0;
    if (!purpose || n <= 0) return { ok: false, reason: 'invalid_commitment' };
    const available = this.availableWorkers(sector);
    if (n > available) return { ok: false, reason: 'insufficient_crew', available, need: n };
    const c = sector.workforce.committed;
    c[purpose] = (Number(c[purpose]) || 0) + n;
    this.log.write('workers_committed', { sector: sectorCode, purpose, count: n, committed: { ...c }, available_after: this.availableWorkers(sector) });
    this.touch();
    return { ok: true, committed: n, available_after: this.availableWorkers(sector) };
  }

  /** Give a purpose's workers back. Returns how many came back. */
  releaseWorkers(sectorCode, purpose) {
    const sector = this.state.sectors[sectorCode];
    if (!sector) return 0;
    const c = sector.workforce.committed;
    const n = Number(c[purpose]) || 0;
    if (!n) return 0;
    delete c[purpose];
    this.log.write('workers_released', { sector: sectorCode, purpose, count: n, available_after: this.availableWorkers(sector) });
    this.touch();
    return n;
  }

  /**
   * A commitment cannot outlive the workers holding it. If injuries or a
   * facilitator's hand shrink the active pool below what is committed, the
   * shortfall is released — largest claim first — and said out loud. The
   * action that owned those workers is told by its own reducer; this only
   * keeps the count honest.
   */
  reconcileCommitments(sectorCode) {
    const sector = this.state.sectors[sectorCode];
    if (!sector) return 0;
    const c = sector.workforce.committed || {};
    let over = this.committedWorkers(sector) - sector.workforce.active;
    if (over <= 0) return 0;
    let freed = 0;
    for (const purpose of Object.keys(c).sort((a, b) => (Number(c[b]) || 0) - (Number(c[a]) || 0))) {
      if (over <= 0) break;
      const take = Math.min(over, Number(c[purpose]) || 0);
      c[purpose] -= take;
      if (c[purpose] <= 0) delete c[purpose];
      over -= take;
      freed += take;
    }
    if (freed > 0) {
      this.log.write('workers_commitment_trimmed', { sector: sectorCode, released: freed, committed: { ...c } });
      this.touch();
    }
    return freed;
  }
  ...
  /** Workers a sector has claimed for an action that holds them. */
  committedWorkers(sector) {
    const c = (sector.workforce && sector.workforce.committed) || {};
    return Object.values(c).reduce((a, n) => a + (Number(n) || 0), 0);
  }

  /**
   * Workers a sector can actually put on anything right now — the one place
   * the engine answers that question, for repairs, upgrades, a restart and
   * every projection alike. Injured workers are already out of `active`;
   * committed ones are in it but spoken for.
   */
  availableWorkers(sector) {
    let n = sector.workforce.active - this.committedWorkers(sector);
    if (sector.status === 'BROWNOUT') {
      let held = Number(economy.brownoutEffects(this.cfg, sector.code).worker_penalty ?? 0);
      if (this.preserving(sector.code, 'PRESERVE_CREW')) {
        held -= Number((this.brownoutPreserveConfig() || {}).crew_released ?? 1);
      }
      n -= Math.max(0, held);
    }
  ...
  adjustWorkforce(code, activeDelta = 0, injuredDelta = 0) {
    ...
    this.reconcileCommitments(code);
  ...
  trnCapacity() {
    ...
    if (trn && trn.status === 'BROWNOUT') {
      ...
      if (this.preserving('TRN', 'PRESERVE_LOGISTICS')) {
        cap = Math.max(cap, Number((this.brownoutPreserveConfig() || {}).logistics_capacity ?? 2));
      }
    }
  ...
  requestTransfer({ from, to, resource, amount, note = '', by = 'sector' }) {
    ...
    const request = {
      ...
      // Game time it was raised: an explicit timeout, when a scenario sets one, counts from here.
      timeout_from_game_s: this.state.game_clock_s,
  ...
  fulfillRequest(id, { by = 'facilitator' } = {}) {
    ...
    if (!r) return { ok: false, reason: 'unknown_request' };
    if (r.status === 'EXPIRED') return { ok: false, reason: 'expired' };
    // v20: a second ACCEPT — a double click, a retried message, a reconnect —
    // hands back the transfer this request already made. One acceptance, one
    // transfer, however many times the button is pressed.
    if (r.status === 'TRANSFER_CREATED' && r.transfer_id) {
      const already = this.findTransfer(r.transfer_id);
      if (already) return { ok: true, request: r, transfer: already, duplicate: true };
    }
    if (r.status !== 'REQUESTED') return { ok: false, reason: 'request_closed', status: r.status };
    const override = by === 'facilitator';
    if (!override && by !== r.supplier) return { ok: false, reason: 'not_supplier', supplier: r.supplier };
   ...
   */
  approveTransfer(id, { by = 'TRN', force = false, bypassCapacity = false, dryRunCapacity = false } = {}) {
    const t = this.findTransfer(id);
    if (!t) return { ok: false, reason: 'unknown_transfer' };
    ...
    if (!force && !bypassCapacity && !dryRunCapacity && used >= cap) return refuse('capacity', { capacity: cap, used, basis });
    ...
    // The valve asks the question without answering it: everything that could
    // refuse this approval has now been checked, and nothing has moved yet.
    if (dryRunCapacity) return { ok: true, dry_run: true };
  ...
  /**
   * The number in a period stamp. Since continuous gameflow a stamp is an
   * OPERATING CYCLE (C3 → 3); a round id (R2 → 2) still reads, because a log
   * or a snapshot from before the change carries one.
   */
  roundNumber(id = this.periodKey()) {
    const m = /^([RC])(\d+)$/.exec(String(id || ''));
    if (m) return Number(m[2]);
    const i = this.rounds.rounds.findIndex((r) => r.id === id);
    return i < 0 ? 0 : i;
  }

  /** CURRENT / STALE / OUTDATED by cycle distance, NOT UPDATED when never set. */
  ...
  /**
   * What COMM may claim a sector's condition is. Deliberately the same four
   * words the system uses, because the interesting moment is when COMM reports
   * STABLE for a sector the wall is drawing CRITICAL — and a room that can
   * see both learns something about how information travels under pressure.
   */
  static get REPORTED_STATUSES() { return ['STABLE', 'DEGRADED', 'CRITICAL', 'DARK']; }

  /** How long a sector stays lit when COMM points at it. */
  static get FOCUS_SECONDS() { return 8; }

  emptyBroadcast() {
    const rows = {};
    for (const code of Object.keys(this.state ? this.state.sectors : {})) {
      rows[code] = { power: null, water: null, med: null, parts: null, status: null, round: null, report_round: null };
    }
  ...
  setBroadcastRow(sectorCode, values = {}, { by = 'COM' } = {}) {
    ...
    // The reported condition. It is a claim about the sector, not the sector:
    // nothing here reads or writes integrity, status, faults or stock.
    if (values.status !== undefined) {
      const word = values.status === null || values.status === '' ? null : String(values.status).toUpperCase();
      if (word !== null && !GameState.REPORTED_STATUSES.includes(word)) {
        return { ok: false, reason: 'invalid_status', field: 'status', allowed: GameState.REPORTED_STATUSES };
      }
      row.status = word;
    }
    row.round = this.periodKey();
    /*
      TWO STAMPS, TWO QUESTIONS. `round` is the operating cycle, and it is what
      freshness is measured in — how long ago, in the beat the economy runs on.
      `report_round` is the ROUND the room was in when COMM filed this, and it
      is what the Big Screen tags the numbers with. The tag must never follow
      the clock: a report filed in Round 1 still reads R1 in Round 2, because
      the whole point of the board is that it can be out of date.
    */
    row.report_round = this.state.round;
  ...
  /*
   * SECTOR FOCUS — COMM pointing at one sector on the shared screen.
   *
   * Eight seconds of emphasis and nothing else. It changes no health, no
   * status, no fault and no stock; it cannot hide a brownout, a DARK sector or
   * a critical edge, because those are drawn by the system underneath it. One
   * at a time: a second focus replaces the first rather than lighting the map
   * up like a christmas tree.
   */
  focusView() {
    const f = this.state.broadcast ? this.state.broadcast.focus : null;
    if (!f) return null;
    const left = f.expires_at_s - Number(this.state.game_clock_s || 0);
    if (left <= 0) return null;
    return { sector: f.sector, remaining_s: Math.ceil(left), duration_s: f.duration_s };
  }

  setSectorFocus(sectorCode, { by = 'COM' } = {}) {
    const code = String(sectorCode || '').toUpperCase();
    const override = by === 'facilitator';
    if (!override && by !== 'COM') return { ok: false, reason: 'com_edit_forbidden', actor: by };
    if (!this.state.sectors[code]) return { ok: false, reason: 'unknown_sector' };
    const seconds = Number(this.cfg.sector_focus_seconds ?? GameState.FOCUS_SECONDS);
    const now = Number(this.state.game_clock_s || 0);
    const previous = this.state.broadcast.focus ? this.state.broadcast.focus.sector : null;
    this.state.broadcast.focus = {
      sector: code, by, duration_s: seconds,
      started_at_s: now, expires_at_s: now + seconds, started_at: new Date().toISOString(),
    };
    this.log.write('com_sector_focus_started', { sector: code, by, seconds, previous, facilitator_override: override });
    this.touch();
    return { ok: true, focus: this.focusView() };
  }

  clearSectorFocus({ by = 'system' } = {}) {
    const f = this.state.broadcast ? this.state.broadcast.focus : null;
    if (!f) return { ok: false, reason: 'no_focus' };
    this.state.broadcast.focus = null;
    this.log.write('com_sector_focus_ended', { sector: f.sector, by });
    this.touch();
    return { ok: true, sector: f.sector };
  }

  /** The eight seconds running out. Called by the tick; expiry is not a state change. */
  tickSectorFocus() {
    const f = this.state.broadcast ? this.state.broadcast.focus : null;
    if (!f) return;
    if (Number(this.state.game_clock_s || 0) >= f.expires_at_s) this.clearSectorFocus({ by: 'expiry' });
  }

  /** One city-wide priority line. A new one replaces the old. */
  // -- COM'S CITY PRIORITY (P1) -----------------------------------------------
  /*
   * One category, named by Comms, shown on the wall. It is a communication
   * aid and nothing else: it resolves no fault, moves no stock and discloses
   * no hidden value. The cooldown is what makes it a judgement rather than a
   * running commentary — COM has to decide the moment is worth spending.
   */

  comPriorityConfig() {
    const c = this.cfg.com_intelligence_priority || {};
    return c.enabled === false ? null : c;
  }

  comPriorityCategories() {
    const cfg = this.comPriorityConfig();
    return cfg ? (cfg.categories || []) : [];
  }

  comPriorityView() {
    const cfg = this.comPriorityConfig();
    if (!cfg) return null;
    const p = this.state.broadcast.priority || null;
    const cool = Number(cfg.cooldown_s) || 0;
    const since = p ? Number(this.state.game_clock_s || 0) - Number(p.set_at_s || 0) : Infinity;
    const wait = Math.max(0, Math.ceil(cool - since));
    return {
      current: p ? { ...p } : null,
      categories: this.comPriorityCategories(),
      cooldown_s: cool,
      ready: wait <= 0,
      ready_in_s: wait === Infinity ? 0 : wait,
    };
  }

  setCityPriority(categoryId, { by = 'COM' } = {}) {
    const cfg = this.comPriorityConfig();
    if (!cfg) return { ok: false, reason: 'priority_disabled' };
    const override = by === 'facilitator';
    if (!override && by !== 'COM') return { ok: false, reason: 'com_edit_forbidden', actor: by };
    const cat = this.comPriorityCategories().find((c) => c.id === String(categoryId || '').toUpperCase());
    if (!cat) return { ok: false, reason: 'unknown_category', categories: this.comPriorityCategories().map((c) => c.id) };
    const view = this.comPriorityView();
    if (!override && !view.ready) return { ok: false, reason: 'cooldown', ready_in_s: view.ready_in_s };

    const previous = this.state.broadcast.priority || null;
    this.state.broadcast.priority = {
      category: cat.id,
      label: cat.label || cat.id,
      by,
      period: this.periodKey(),
      set_at_s: Number(this.state.game_clock_s || 0),
      set_at: new Date().toISOString(),
    };
    this.log.write('com_intelligence_priority', {
      category: cat.id, by, previous: previous ? previous.category : null,
      period: this.periodKey(), facilitator_override: override,
    });
    this.ticker('announce', `COM PRIORITY: ${cat.label || cat.id}`);
    this.touch();
    return { ok: true, priority: { ...this.state.broadcast.priority }, view: this.comPriorityView() };
  }

  clearCityPriority({ by = 'COM' } = {}) {
    const override = by === 'facilitator';
    if (!override && by !== 'COM') return { ok: false, reason: 'com_edit_forbidden', actor: by };
    if (!this.state.broadcast.priority) return { ok: false, reason: 'no_priority' };
    const was = this.state.broadcast.priority.category;
    this.state.broadcast.priority = null;
    this.log.write('com_intelligence_priority_cleared', { category: was, by, facilitator_override: override });
    this.touch();
    return { ok: true, view: this.comPriorityView() };
  }
  ...
  agrDraw(roundId, { salt = 0 } = {}) {
    ...
    // A balanced hand: one slot per bucket first, from the fresh pile where it
    // can and the repeats where it must, then the rest drawn as before. Every
    // pick still comes off the same seeded shuffle, so a run is reproducible
    // and a reroll (a different salt) still gives a genuinely different hand.
    const hand = [];
    const take = (list) => { for (const id of list) if (!hand.includes(id)) { hand.push(id); return true; } return false; };
    const bal = this.cfg.agr_balanced_draw || {};
    if (bal.enabled !== false && Array.isArray(bal.buckets)) {
      const categoryOf = (id) => { const c = this.agrCard(id); return c ? c.category : null; };
      for (const bucket of bal.buckets) {
        if (hand.length >= n) break;
        const wanted = new Set(bucket.categories || []);
        const inBucket = (list) => shuffle(list.filter((id) => wanted.has(categoryOf(id))));
        // a fresh card for the bucket, or a repeat rather than an empty slot
        if (!take(inBucket(fresh))) take(inBucket(repeats));
      }
    }
    for (const id of shuffle(fresh)) { if (hand.length >= n) break; take([id]); }
    for (const id of shuffle(repeats)) { if (hand.length >= n) break; take([id]); }
    return { hand: [...new Set(hand)].slice(0, n), seed };
  }

  /** Deal for the current round if it has no offer yet. Idempotent. */
  agrEnsureOffer({ by = 'system' } = {}) {
    const agr = this.state.agr;
    if (agr.round === this.periodKey() && agr.offered.length) return agr;
    ...
    const { hand, seed } = this.agrDraw(this.periodKey());
    agr.round = this.periodKey();
    agr.offered = hand;
    agr.selected = null;
    agr.used = false;
    agr.target = null;
  ...
  agrSelect(cardId, { by = 'AGR' } = {}) {
    ...
    const agr = this.state.agr;
    if (agr.round !== this.periodKey() || !agr.offered.length) return { ok: false, reason: 'agr_offer_not_ready' };
  ...
  agrActivate(cardId, { by = 'AGR', target = null, force = false } = {}) {
    ...
    if (agr.round !== this.periodKey() || !agr.offered.length) return refuse('agr_offer_not_ready');
    ...
    switch (fx.type) {
      ...
      case 'capacity': {
        ...
        const e = this.addEffect({
          ...
          expires: 'round', round: this.periodKey(), label: card.title,
        });
  ...
  agrReroll({ by = 'facilitator' } = {}) {
    ...
    const { hand, seed } = this.agrDraw(this.periodKey(), { salt: agr.reroll_count });
    agr.offered = hand;
    agr.selected = null;
    agr.generated_at = new Date().toISOString();
   ...
   */
  expirePendingTransfers({ reason = 'round_change', fromRound = null } = {}) {
    const now = new Date().toISOString();
    let n = 0;
    ...
    // A supplier has already said yes to these, so they survive the boundary
    // and come back DELAYED — still needing one of TRN's slots to complete.
    if (this.cfg.carry_accepted_transfers_on_round_change !== false) {
      let carried = 0;
      for (const t of this.state.transfers) {
        if (!GameState.OPEN_TRANSFER.includes(t.status)) continue;
        t.delayed = true;
        t.delayed_from_round = t.delayed_from_round || fromRound || this.state.round;
        t.updated_at = now;
        carried += 1;
        this.log.write('transfer_delayed', {
          id: t.id, from: t.from, to: t.to, resource: t.resource, amount: t.amount,
          status: t.status, from_round: t.delayed_from_round, reason,
        });
      }
      if (carried) {
        this.ticker('transfer', `${carried} accepted transfer${carried === 1 ? '' : 's'} carried over as DELAYED`, { scope: 'admin' });
        this.touch();
      }
    } else {
  ...
  setPhase(id) {
    ...
    // The end of the simulation is the one phase that stops time. Every other
    // phase change happens underneath a running clock and a working city.
    if (def.mode === 'ENDED') this.stopLiveSystems();
    this.log.write('phase', { phase: def.id, from: prev, round: this.state.round, mode: def.mode });
    // A phase is a backend pacing marker: the facilitator is told, the room is not.
    this.ticker('phase', `${def.name.toUpperCase()}`, { scope: 'admin' });
    this.touch();
    return true;
  }

  /**
   * END OF SIMULATION. Live timers stop and the final city state stands as it
   * is: nothing is cleared, refilled, healed or scored away, and no debrief or
   * reflection screen is launched at anybody.
   */
  stopLiveSystems() {
  ...
  /** The facilitator ending the run by hand, from wherever the phase list is. */
  endSimulation({ by = 'facilitator' } = {}) {
    const last = (this.rounds.phases || [])[(this.rounds.phases || []).length - 1];
    if (!last) return { ok: false, reason: 'no_end_phase' };
    if (this.state.phase === last.id) return { ok: true, phase: last.id, already: true };
    this.setPhase(last.id);
    this.log.write('simulation_ended', { by, phase: last.id, game_clock_s: Math.round(this.state.game_clock_s) });
    this.ticker('phase', 'SIMULATION ENDED');
    return { ok: true, phase: last.id };
  }
  ...
  /**
   * CONTINUOUS GAMEFLOW (2026-09-21). A round is now nothing but the inject
   * group an internal phase draws from: moving into one arms its timeline and
   * says so in the log, and that is the whole of it. It charges no upkeep,
   * touches no clock, refreshes no allowance and expires no paperwork — the
   * OPERATING CYCLE does all of that on its own timer, so the city a team has
   * built (or broken) crosses every phase boundary exactly as it stood.
   * Nothing about a round reaches a participant screen.
   */
  setRound(id) {
    ...
    this.log.write('round', { round: cfg.id, continuous: true });
    // The facilitator's own feed says the number too: the round's name in
    // this file is history, and printing it is how a title reaches a screen.
    this.ticker('round', `ROUND ${this.roundOrdinal(cfg.id)}`, { scope: 'admin' });
    this.touch();
  }

  /**
   * THE OPERATING CYCLE'S refresh, the one beat where the city's periodic
   * allowances come back. Called by economy.processCycle after production and
   * upkeep, never by a phase change. An operating cycle is not a round: it
   * pauses nothing, shows nothing, and interrupts no one.
   */
  cycleRefresh({ by = 'cycle' } = {}) {
    // A pending generator upgrade lands first: its crew must be back in the
    // pool, and its new level in place, before anything reads either.
    this.completeGeneratorUpgrades();
    // A restart crew is released on the same beat.
    for (const code of Object.keys(this.state.sectors)) {
      if (this.releaseWorkers(code, 'EMERGENCY_RESTART')) {
        this.log.write('emergency_restart_completed', { sector: code, cycle: this.state.cycle.number });
      }
    }
    rewards.settlePending(this);
    // Both counters: the basis is the cycle now, and a stale round counter
    // must never be what refuses Transport its next approval.
    this.resetStamps('all', { by });
    this.resetHeals({ by });
    // Bonuses that lasted "until the period ends" end. AGR is dealt again.
    this.expireRoundEffects();
    this.agrEnsureOffer({ by });
    this.touch();
  }
  ...
  clock(action, seconds, which = 'round', { reason = null, by = 'facilitator' } = {}) {
    ...
    if (action === 'start' || action === 'resume') {
      ...
      // Time only: the configured length of the whole live shift. The phase,
      // the trays, health, faults, workers, allowances and AGR stay exactly
      // where they are — resetting MASTER TIME is not restarting the city.
      clk.remaining_s = which === 'council' ? Number(this.cfg.council_length_s ?? clk.remaining_s) : this.liveLength();
    } else {
   ...
   */
  // -- TRN EMERGENCY MOVEMENT (P1) --------------------------------------------
  /*
   * One approval a round beyond the limit, paid for in Transport's own health.
   * The cost is what keeps the bottleneck honest: a table that spends it is
   * choosing this delivery over its own condition, and has to say so first.
   */

  trnEmergencyConfig() {
    const c = this.cfg.trn_emergency_movement || {};
    return c.enabled === false ? null : c;
  }

  trnEmergencyView() {
    const cfg = this.trnEmergencyConfig();
    const trn = this.state.sectors.TRN;
    if (!cfg || !trn) return null;
    const used = trn.emergency_movement_round === this.periodKey();
    return {
      available: !used && trn.status !== 'DARK',
      used_this_round: used,
      health_cost: Number(cfg.health_cost) || 5,
      trn_health: Math.round(trn.integrity),
      health_after: Math.max(0, Math.round(trn.integrity) - (Number(cfg.health_cost) || 5)),
    };
  }

  /**
   * Spend the valve on one transfer. Charged first and only once: if the
   * approval that follows is refused for some other reason, the health is
   * already gone, so the check order below puts every other refusal first.
   */
  useEmergencyMovement(id, { by = 'TRN' } = {}) {
    const cfg = this.trnEmergencyConfig();
    if (!cfg) return { ok: false, reason: 'emergency_movement_disabled' };
    const view = this.trnEmergencyView();
    if (!view.available) return { ok: false, reason: view.used_this_round ? 'emergency_used_this_round' : 'trn_unavailable' };
    const t = this.findTransfer(id);
    if (!t) return { ok: false, reason: 'unknown_transfer' };

    // Everything the ordinary approval would refuse for, checked BEFORE the
    // health is spent — the valve buys capacity, not a pass on the rules.
    const dry = this.approveTransfer(id, { by, dryRunCapacity: true });
    if (!dry.ok) return dry;

    const trn = this.state.sectors.TRN;
    const cost = Number(cfg.health_cost) || 5;
    const before = Math.round(trn.integrity);
    this.setIntegrity('TRN', trn.integrity - cost);
    trn.emergency_movement_round = this.periodKey();

    const res = this.approveTransfer(id, { by, bypassCapacity: true });
    this.log.write('trn_emergency_movement_used', {
      transfer: id, by, health_cost: cost, health_before: before,
      health_after: Math.round(this.state.sectors.TRN.integrity),
      round: this.state.round, approved: !!res.ok, reason: res.ok ? null : res.reason,
    });
    this.ticker('transfer', `TRN EMERGENCY MOVEMENT — ${cost} HEALTH`);
    this.touch();
    return { ...res, emergency: true, health_cost: cost, trn_health: Math.round(this.state.sectors.TRN.integrity) };
  }

  // -- BROWNOUT: ONE THING STAYS STANDING (P1) --------------------------------
  /*
   * A brownout takes the same things it always did. What it now also does is
   * ask the sector which one of them to keep. Every option is written against
   * a penalty this engine really applies, so the choice is a trade-off rather
   * than a menu: the crew brownout holds back, the output it halves, TRN's
   * single approval, COM's telemetry, and the slow bleed under all of them.
   */

  brownoutPreserveConfig() {
    const c = this.cfg.brownout_preserve || {};
    return c.enabled === false ? null : c;
  }

  /** The options this sector may choose between, or [] when it has none. */
  brownoutOptions(code) {
    const cfg = this.brownoutPreserveConfig();
    if (!cfg) return [];
    const o = cfg.options || {};
    return o[code] || o.default || [];
  }

  /** Is `fn` in force for this sector right now? Only ever true in brownout. */
  preserving(code, fn) {
    const s = this.state.sectors[code];
    return !!s && s.status === 'BROWNOUT' && s.preserved_function === fn;
  }

  brownoutView(code) {
    const s = this.state.sectors[code];
    const cfg = this.brownoutPreserveConfig();
    if (!s || !cfg || s.status !== 'BROWNOUT') return null;
    const labels = cfg.labels || {};
    return {
      chosen: s.preserved_function,
      options: this.brownoutOptions(code).map((id) => ({ id, label: labels[id] || id })),
    };
  }

  /**
   * The sector picks, once per brownout. Changing it mid-brownout is allowed —
   * the situation changes, and a locked-in wrong guess is the kind of trap this
   * whole brief exists to remove.
   */
  chooseBrownoutFunction(code, fn, { by = code } = {}) {
    const s = this.state.sectors[code];
    if (!s) return { ok: false, reason: 'unknown_sector' };
    if (s.status !== 'BROWNOUT') return { ok: false, reason: 'not_in_brownout' };
    const allowed = this.brownoutOptions(code);
    if (!allowed.includes(fn)) return { ok: false, reason: 'invalid_function', options: allowed };
    const before = s.preserved_function;
    s.preserved_function = fn;
    this.log.write('brownout_function_selected', { sector: code, function: fn, previous: before, by, round: this.state.round });
    this.ticker('status', `${code} PRESERVES ${fn.replace('PRESERVE_', '')}`, { scope: 'admin' });
    this.touch();
    return { ok: true, chosen: fn, view: this.brownoutView(code) };
  }

  // -- THE PRESSURE REGULATOR (P0) --------------------------------------------
  /*
   * AMBER is the target: stretched, still deciding. These are the levers for
   * getting back to it from RED, and none of them fires on its own — the
   * facilitator keeps the judgement, and every use is logged with a reason so
   * the debrief can separate what a team did from what it was given.
   */

  decayFrozen() {
    const p = this.state.pressure || {};
    return Number(p.decay_freeze_until_s || 0) > Number(this.state.game_clock_s || 0);
  }

  /** What the console shows about the regulator right now. */
  pressureView() {
    const p = this.state.pressure || {};
    const left = Math.max(0, Number(p.decay_freeze_until_s || 0) - Number(this.state.game_clock_s || 0));
    return {
      faults_paused: !!p.faults_paused,
      decay_frozen: this.decayFrozen(),
      decay_freeze_remaining_s: Math.round(left),
      trn_capacity: this.trnCapacity(),
      trn_capacity_granted: this.state.effects.filter((e) => e.kind === 'trn_capacity').reduce((a, e) => a + (Number(e.delta) || 0), 0),
      next_scheduled_in_s: this.state.scheduled.length
        ? Math.max(0, Math.round(this.state.scheduled[0].at_s - (this.state.game_clock_s || 0)))
        : null,
    };
  }

  /**
   * One door for every relief action, so one event type covers them all and
   * nothing can be granted without saying why.
   */
  pressureRelief(kind, payload = {}, { by = 'facilitator', reason = '' } = {}) {
    const p = this.state.pressure;
    let detail = null;
    ...
    switch (String(kind)) {
      case 'pause_faults':
      case 'resume_faults': {
        p.faults_paused = kind === 'pause_faults';
        detail = { faults_paused: p.faults_paused };
        this.ticker('event', `FAULT INJECTION ${p.faults_paused ? 'HELD' : 'RESUMED'}`, { scope: 'admin' });
        break;
      }
      case 'freeze_decay': {
        const secs = Math.max(0, Math.round(Number(payload.seconds) || 0));
        if (!secs) return { ok: false, reason: 'seconds_required' };
        p.decay_freeze_until_s = Number(this.state.game_clock_s || 0) + secs;
        detail = { seconds: secs, until_s: p.decay_freeze_until_s };
        this.ticker('event', `HEALTH DECAY FROZEN FOR ${secs}s`, { scope: 'admin' });
        break;
      }
      case 'trn_capacity': {
        const delta = Math.round(Number(payload.delta) || 1);
        if (delta === 0) return { ok: false, reason: 'delta_required' };
        const e = this.addEffect({
          kind: 'trn_capacity', delta, expires: 'round', round: this.periodKey(),
          label: `FACILITATOR +${delta} APPROVAL`, source: 'pressure_relief',
        });
        detail = { delta, effect_id: e.id, capacity_now: this.trnCapacity() };
        this.ticker('transfer', `TRN CAPACITY +${delta} THIS CYCLE`, { scope: 'admin' });
        break;
      }
      case 'delay_inject': {
        const secs = Math.max(0, Math.round(Number(payload.seconds) || 60));
        const next = this.state.scheduled[0];
        if (!next) return { ok: false, reason: 'nothing_scheduled' };
        next.at_s += secs;
        this.state.scheduled.sort((a, b) => a.at_s - b.at_s);
        detail = { seconds: secs, id: next.id, kind: next.kind, at_s: next.at_s };
        this.ticker('event', `NEXT INJECT DELAYED ${secs}s`, { scope: 'admin' });
        break;
      }
      case 'hold_fault': {
        // There are no per-fault deadlines in this engine — decay is the only
        // clock a fault has — so the equivalent relief is to stop its bleed.
        const f = this.findFault(String(payload.sector || '').toUpperCase(), String(payload.fault_code || '').toUpperCase());
        if (!f) return { ok: false, reason: 'unknown_fault' };
        f.paused = true;
        detail = { sector: payload.sector, fault_code: payload.fault_code, paused: true };
        this.ticker('fault', `${payload.fault_code} DECAY HELD`, { scope: 'admin' });
        break;
      }
      case 'agr_recovery_card': {
        const cats = payload.categories || ['health', 'workforce'];
        const r = this.agrGrantFromCategories(cats, { by });
        if (!r.ok) return r;
        detail = { card: r.card, categories: cats };
        break;
      }
      default:
        return { ok: false, reason: 'unknown_relief' };
    }

    this.log.write('facilitator_pressure_relief', {
      kind, by, reason, round: this.state.round, phase: this.state.phase, ...detail,
    });
    this.touch();
    return { ok: true, kind, ...detail, pressure: this.pressureView() };
  }

  /**
   * Put one recovery-shaped card into AGR's hand. The pools are balanced
   * properly in P1; this only guarantees the facilitator can hand one over.
   */
  agrGrantFromCategories(categories, { by = 'facilitator' } = {}) {
    const agr = this.state.agr;
    if (!agr) return { ok: false, reason: 'no_agr' };
    const pool = this.agrPool().filter((c) => c.enabled && categories.includes(c.category)
      && !(agr.offered || []).some((o) => (o.id || o) === c.id));
    if (!pool.length) return { ok: false, reason: 'no_card_available' };
    const card = pool[0];
    agr.offered = [...(agr.offered || []), card];
    this.log.write('agr_card_granted', { card: card.id, category: card.category, by });
    this.ticker('event', `AGR GRANTED ${card.title}`, { scope: 'admin' });
    this.touch();
    return { ok: true, card: card.id };
  }

  // -- EMERGENCY SHUTDOWN / RESTART (P0) --------------------------------------
  /*
   * Zero health used to be the end of a table's game: no repair, no output, no
   * approvals, no healing, and no way up. The status itself is unchanged — the
   * wall still says DARK / OFFLINE — but the sector now has one route back that
   * costs enough to be a real decision and leaves it at CRITICAL, not healthy.
   */

  emergencyRestartConfig() {
    const c = this.cfg.emergency_restart || {};
    return c.enabled === false ? null : c;
  }

  /** Everything the console needs to offer the restart, and the reducer checks. */
  emergencyRestartView(code) {
    const cfg = this.emergencyRestartConfig();
    const s = this.state.sectors[code];
    if (!cfg || !s) return null;
    if (s.status !== 'DARK') return null;
    const cost = cfg.cost || {};
    const workers = Number(cfg.workers) || 0;
    const available = this.availableWorkers(s);
    const short = {};
    for (const [k, v] of Object.entries(cost)) {
      const have = Number(s.inventory[k]) || 0;
      if (have < v) short[k] = v - have;
    }
    const running = Number((s.workforce.committed || {}).EMERGENCY_RESTART) || 0;
    const blockers = [];
    if (running) blockers.push('restart_running');
    if (Object.keys(short).length) blockers.push('insufficient_stock');
    if (available < workers) blockers.push('insufficient_crew');
    return {
      cost: { ...cost },
      workers_required: workers,
      workers_available: available,
      health_after: Number(cfg.health_after) || 20,
      short,
      running: running > 0,
  ...
  /**
   * Pay, and the lights come back at the floor of CRITICAL. The crew stays on
   * the restart until the round ends — the work is not over when the power is
   * on — so the sector comes back able to act, but not able to do everything.
   */
  emergencyRestart(code, { by = code } = {}) {
    const cfg = this.emergencyRestartConfig();
    if (!cfg) return { ok: false, reason: 'restart_disabled' };
    const s = this.state.sectors[code];
    if (!s) return { ok: false, reason: 'unknown_sector' };
    if (s.status !== 'DARK') return { ok: false, reason: 'not_shut_down' };
    const view = this.emergencyRestartView(code);
    if (!view.can_start) return { ok: false, reason: view.blockers[0], blockers: view.blockers, view };

    const crew = this.commitWorkers(code, 'EMERGENCY_RESTART', view.workers_required);
    if (!crew.ok) return { ok: false, reason: crew.reason, available: crew.available };
    for (const [k, v] of Object.entries(view.cost)) {
      s.inventory[k] = Math.max(0, (Number(s.inventory[k]) || 0) - v);
    }
    // The floor is set by config, and the status word follows from it as it
    // does for every other health change — no special-case state.
    this.setIntegrity(code, view.health_after);

    this.log.write('emergency_restart', {
      sector: code, by, cost: { ...view.cost }, workers_committed: view.workers_required,
      integrity_after: this.state.sectors[code].integrity, status_after: this.state.sectors[code].status,
      round: this.state.round,
    });
    this.ticker('status', `${code} EMERGENCY RESTART — BACK AT ${view.health_after}%`);
    this.touch();
    return {
      ok: true, integrity: this.state.sectors[code].integrity,
      status: this.state.sectors[code].status, workers_committed: view.workers_required,
    };
  }

  // -- GENERATORS (P0) ------------------------------------------------------------
  /*
   * A generator has an installed LEVEL. The level sets the base output; every
   * existing modifier still applies on top of it, so a brownout still halves a
   * Level 4 generator and POW still scales with the core. Upgrading is optional
   * — Level 2 is the production the scenario always had.
   *
   * An upgrade is not a gamble. It cannot fail, it cannot skip a level, and it
   * cannot start unless everything it needs is present. What it costs is Parts,
   * which are gone, and workers, who are held until the round ends.
   */

  generatorConfig() {
    const g = this.cfg.generator_upgrades || {};
    return g.enabled === false ? null : g;
  }

  /**
   * The sector's generator, or null. This is a READ — the projections call it
   * on every frame — so it never writes. initGenerators() does the building.
   */
  generatorFor(code) {
    const cfg = this.generatorConfig();
    const s = this.state.sectors[code];
    if (!cfg || !s) return null;
    if (!(cfg.sectors || []).includes(code)) return null;
    return s.generator || null;
  }

  /**
   * Give every generator sector a generator. Called once with the run and once
   * on restore, and safe to call again: it fills only what is missing, so a
   * scenario that turns generators on mid-run picks them up without a reset.
   */
  initGenerators() {
    const cfg = this.generatorConfig();
    if (!cfg) return 0;
    let made = 0;
    for (const code of cfg.sectors || []) {
      const s = this.state.sectors[code];
      if (!s || s.generator) continue;
      s.generator = {
        level: Number(cfg.start_level) || 2,
        upgrade_used_round: null,
        pending: null,
        confirmation: null,
      };
      made += 1;
  ...
  generatorLevelInfo(level) {
    const cfg = this.generatorConfig();
    if (!cfg) return null;
    return (cfg.levels || []).find((l) => Number(l.level) === Number(level)) || null;
  }

  /** The cost row for reaching `toLevel`, or null when there is no such step. */
  generatorUpgradeCost(toLevel) {
    const cfg = this.generatorConfig();
    if (!cfg) return null;
    return (cfg.costs || []).find((c) => Number(c.to_level) === Number(toLevel)) || null;
  }

  /** Which sector must confirm support before `code` may reach `toLevel`. */
  generatorSupportFrom(code, toLevel) {
    const cost = this.generatorUpgradeCost(toLevel);
    const map = cost && cost.support_from;
    return (map && map[code]) || null;
  }

  /** The base a generator sector produces at its installed level. */
  generatorProduction(sector) {
    const g = sector && sector.generator ? sector.generator : null;
    if (!g) return null;
    const info = this.generatorLevelInfo(g.level);
    if (!info) return null;
    const keys = Object.keys(sector.production || {});
    if (keys.length !== 1) return null;   // only a single-line generator
    return { [keys[0]]: Number(info.output) || 0 };
  }

  /** Is the round far enough along for upgrades to be offered at all? */
  generatorUpgradesOpen() {
    const cfg = this.generatorConfig();
    if (!cfg) return false;
    const from = cfg.enabled_from_round;
    if (!from) return true;
    const ids = (this.rounds.rounds || []).map((r) => r.id);
    const i = ids.indexOf(this.state.round);
    const j = ids.indexOf(from);
    return i >= 0 && j >= 0 ? i >= j : true;
  }

  /**
   * Everything a screen needs to show the upgrade decision before it is taken,
   * and everything `startGeneratorUpgrade` will check. One source, so what the
   * console promises and what the reducer enforces cannot drift apart.
   */
  generatorView(code) {
    const g = this.generatorFor(code);
    if (!g) return null;
    const s = this.state.sectors[code];
    const cfg = this.generatorConfig();
    const info = this.generatorLevelInfo(g.level);
    const toLevel = g.level + 1;
    const next = this.generatorLevelInfo(toLevel);
    const cost = this.generatorUpgradeCost(toLevel);
    const supportFrom = this.generatorSupportFrom(code, toLevel);
    const available = this.availableWorkers(s);
    const parts = Number(s.inventory.parts) || 0;
    const usedThisRound = g.upgrade_used_round === this.state.round;
    const confirmed = !!(g.confirmation && g.confirmation.from === supportFrom && g.confirmation.round === this.state.round);

    const blockers = [];
    if (!this.generatorUpgradesOpen()) blockers.push('not_open_yet');
    if (s.status === 'DARK') blockers.push('sector_dark');
    if (g.pending) blockers.push('upgrade_pending');
    if (usedThisRound) blockers.push('used_this_round');
    if (!next || !cost) blockers.push('at_maximum');
    if (cost && parts < cost.parts) blockers.push('insufficient_parts');
    if (cost && available < cost.workers) blockers.push('insufficient_crew');
    if (supportFrom && !confirmed) blockers.push('missing_support');

    return {
      level: g.level,
      level_name: info ? info.name : null,
      max_level: Number(cfg.max_level) || 5,
      output_now: info ? info.output : null,
      next_level: next ? next.level : null,
      next_level_name: next ? next.name : null,
      output_next: next ? next.output : null,
      parts_required: cost ? cost.parts : null,
      workers_required: cost ? cost.workers : null,
      parts_available: parts,
      workers_available: available,
      workers_after: g.pending || !cost ? available : Math.max(0, available - cost.workers),
      used_this_round: usedThisRound,
      support_from: supportFrom,
      support_label: supportFrom ? (cfg.support_labels || {})[code] || null : null,
      support_confirmed: supportFrom ? confirmed : null,
      completes: 'END OF CURRENT ROUND',
      cancel_parts_lost: Number(cfg.cancel_parts_lost) || 1,
      can_start: blockers.length === 0,
      blockers,
  ...
  /**
   * The other sector signs off on the Level 5 step. It is an authorisation,
   * not a resource: it costs the supporter nothing but a decision, and it is
   * good for the round it was given in.
   */
  confirmGeneratorSupport(forCode, { by = 'facilitator' } = {}) {
    const g = this.generatorFor(forCode);
    if (!g) return { ok: false, reason: 'no_generator' };
    const toLevel = g.level + 1;
    const from = this.generatorSupportFrom(forCode, toLevel);
    if (!from) return { ok: false, reason: 'support_not_required' };
    const supporter = this.state.sectors[from];
    if (!supporter || supporter.status === 'DARK') return { ok: false, reason: 'supporter_unavailable' };
    g.confirmation = { from, round: this.state.round, at: new Date().toISOString(), by };
    this.log.write('generator_support_confirmed', { sector: forCode, from, to_level: toLevel, by, round: this.state.round });
    this.ticker('event', `${from} confirms support for ${forCode} generator L${toLevel}`, { scope: 'admin' });
    this.touch();
    return { ok: true, confirmation: { ...g.confirmation } };
  }

  startGeneratorUpgrade(code, { by = code } = {}) {
    const g = this.generatorFor(code);
    if (!g) return { ok: false, reason: 'no_generator' };
    const view = this.generatorView(code);
    if (!view.can_start) {
      return { ok: false, reason: view.blockers[0], blockers: view.blockers, view };
    }
    const s = this.state.sectors[code];
    const toLevel = view.next_level;
    const cost = this.generatorUpgradeCost(toLevel);

    // Parts go now and are not refundable in full; workers are held, not spent.
    const crew = this.commitWorkers(code, 'GENERATOR_UPGRADE', cost.workers);
    if (!crew.ok) return { ok: false, reason: crew.reason, available: crew.available };
    s.inventory.parts = Math.max(0, (Number(s.inventory.parts) || 0) - cost.parts);

    g.pending = {
      from_level: g.level,
      to_level: toLevel,
      parts_spent: cost.parts,
      workers_committed: cost.workers,
      started_round: this.state.round,
      started_at: new Date().toISOString(),
      required_support: view.support_from,
      support_confirmed: !!view.support_confirmed,
      by,
    };
    g.upgrade_used_round = this.state.round;
    g.confirmation = null;   // the authorisation is spent on this upgrade

    this.log.write('generator_upgrade_started', {
      sector: code, by, from_level: g.level, to_level: toLevel,
      parts_spent: cost.parts, workers_committed: cost.workers,
      round: this.state.round, support_from: view.support_from,
      workers_available_after: this.availableWorkers(s),
    });
    this.ticker('event', `${code} generator upgrade started — L${g.level} → L${toLevel}`, { scope: 'admin' });
    this.touch();
    return { ok: true, pending: { ...g.pending }, view: this.generatorView(code) };
  }

  /**
   * Cancelling returns the crew at once and all but one Part: the sunk work.
   * The round's single upgrade opportunity comes back — a team that changes its
   * mind early should not also lose the round.
   */
  cancelGeneratorUpgrade(code, { by = code } = {}) {
    const g = this.generatorFor(code);
    if (!g || !g.pending) return { ok: false, reason: 'no_pending_upgrade' };
    const cfg = this.generatorConfig();
    const s = this.state.sectors[code];
    const p = g.pending;
    const lost = Math.min(Number(cfg.cancel_parts_lost) || 1, p.parts_spent);
    const refund = Math.max(0, p.parts_spent - lost);

    const released = this.releaseWorkers(code, 'GENERATOR_UPGRADE');
    s.inventory.parts = (Number(s.inventory.parts) || 0) + refund;
    g.pending = null;
    if (cfg.cancel_returns_round_slot !== false) g.upgrade_used_round = null;

    this.log.write('generator_upgrade_cancelled', {
      sector: code, by, from_level: p.from_level, to_level: p.to_level,
      parts_refunded: refund, parts_lost: lost, workers_released: released,
      round: this.state.round, slot_returned: cfg.cancel_returns_round_slot !== false,
    });
    this.ticker('event', `${code} generator upgrade cancelled — ${lost} part lost`, { scope: 'admin' });
    this.touch();
    return { ok: true, parts_refunded: refund, parts_lost: lost, workers_released: released, view: this.generatorView(code) };
  }

  /**
   * Round end. A pending upgrade lands: the level goes up by exactly one, the
   * crew comes back, the Parts stay spent. Called from setRound before the
   * per-round allowances reset, so the new level is in place for the round
   * that is starting and the upgrade slot is fresh.
   */
  completeGeneratorUpgrades() {
    const cfg = this.generatorConfig();
    if (!cfg) return [];
    const done = [];
    for (const code of cfg.sectors || []) {
      ...
      g.level = Math.min(max, p.from_level + 1);
      g.pending = null;
      const released = this.releaseWorkers(code, 'GENERATOR_UPGRADE');
      this.log.write('generator_upgrade_completed', {
        sector: code, from_level: p.from_level, to_level: g.level,
        parts_spent: p.parts_spent, workers_released: released,
        started_round: p.started_round, completed_round: this.state.round,
      });
      const info = this.generatorLevelInfo(g.level);
      this.ticker('event', `${code} generator now L${g.level}${info ? ` ${info.name}` : ''}`);
      done.push({ sector: code, level: g.level });
    }
    if (done.length) this.touch();
    return done;
  }

  /**
   * A named fault or a facilitator event can knock a generator down, by one
   * level at a time and never below the floor. Nothing else may move the level.
   */
  damageGeneratorLevel(code, { reason = 'event', by = 'facilitator' } = {}) {
    const g = this.generatorFor(code);
    if (!g) return { ok: false, reason: 'no_generator' };
    const cfg = this.generatorConfig();
    const floor = Number(cfg.min_level) || 1;
    if (g.level <= floor) return { ok: false, reason: 'at_minimum', level: g.level };
    const before = g.level;
    g.level = before - 1;
    const info = this.generatorLevelInfo(g.level);
    this.log.write('generator_level_damaged', { sector: code, from_level: before, to_level: g.level, reason, by });
    this.ticker('status', `${code} generator damaged — L${before} → L${g.level}`);
    this.touch();
    return { ok: true, from_level: before, to_level: g.level, level_name: info ? info.name : null };
  }
  ...
  generateOutput(code, { by = code } = {}) {
    ...
    if (s.round_output && s.round_output.round === this.periodKey()) {
    ...
    s.round_output = { round: this.periodKey(), added, at: new Date().toISOString(), by };
    this.refreshStatus(s);
  ...
  // The BREATHER is gone (continuous gameflow, 2026-09-21). Live play holds no
  // scheduled rest: pressure is eased by injecting less, never by stopping the
  // city. The one full stop is pause() above, and it is an emergency.
  ...
  comBlind() {
    ...
    if (this.preserving('COM', 'PRESERVE_INTELLIGENCE')) return false;
  ...
  /**
   * Decay, clocks and countdowns. The emergency pause, BRIEFING and the end freeze
  ...
  tick(elapsedMs) {
    ...
    for (const sector of Object.values(this.state.sectors)) {
      ...
      for (const fault of sector.faults) {
        ...
        if (frozen) continue;
        if (!fault.paused && !this.decayFrozen() && fault.decay_per_min > 0 && sector.status !== 'DARK') {
      ...
      if (!frozen) {
        ...
        if (brown && !this.preserving(sector.code, 'PRESERVE_SYSTEMS')) {
    ...
    if (!frozen) {
      ...
      this.tickScheduled();
      this.tickTimeline();
      this.tickPaperworkTimeouts();
      this.tickSectorFocus();
    }
  ...
  /**
   * EXPLICIT TIMEOUTS, the replacement for round-end expiry (2026-09-21).
   * Visible rounds are gone, so nothing unfinished may die because a backend
   * phase moved. A request or a transfer expires only if the scenario gives it
   * a timeout of its own; at the default of 0 an agreed transfer stands until
   * it is completed, cancelled or declined, and the TRN queue is never swept.
   */
  tickPaperworkTimeouts() {
    const now = this.state.game_clock_s;
    const sweep = (list, open, seconds, ev) => {
      const limit = Number(seconds || 0);
      if (!(limit > 0)) return 0;
      let n = 0;
      for (const x of list) {
        if (!open.includes(x.status)) continue;
        // Game time, like every other countdown here: an emergency pause holds
        // a timeout exactly as it holds a fault. An item raised before the
        // scenario had a timeout starts counting when the timeout appears.
        if (typeof x.timeout_from_game_s !== 'number') x.timeout_from_game_s = now;
        if (now - x.timeout_from_game_s < limit) continue;
        x.status = 'EXPIRED';
        x.expired_at = new Date().toISOString();
        x.updated_at = x.expired_at;
        x.expire_reason = 'timeout';
        n += 1;
        this.log.write(ev, {
          id: x.id, from: x.from ?? x.sector, to: x.to ?? 'MED',
          resource: x.resource ?? null, amount: x.amount ?? null,
          worker_id: x.worker_id ?? null, reason: 'timeout', timeout_s: limit,
        });
      }
      return n;
    };
    const n = sweep(this.state.requests, GameState.OPEN_REQUEST, this.cfg.request_timeout_s, 'request_expired')
      + sweep(this.state.transfers, GameState.OPEN_TRANSFER, this.cfg.transfer_timeout_s, 'transfer_expired');
    if (n) {
      this.ticker('transfer', `${n} item${n === 1 ? '' : 's'} timed out`, { scope: 'admin' });
      this.touch();
    }
    return n;
  }
