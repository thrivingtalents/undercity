    ...
    if (changed) {
      ...
      for (const code of Object.keys(this.state.sectors)) {
        if (this.releaseWorkers(code, 'EMERGENCY_RESTART')) {
          this.log.write('emergency_restart_completed', { sector: code, round: this.state.round });
        }
      }
      rewards.settlePending(this);
      this.resetStamps('round', { by: 'round_change' });
      this.resetHeals({ by: 'round_change' });
      this.expirePendingTransfers({ reason: 'round_change', fromRound: prev });
  ...
  pressureRelief(kind, payload = {}, { by = 'facilitator', reason = '' } = {}) {
    ...
    switch (String(kind)) {
      ...
      case 'trn_capacity': {
        ...
        const e = this.addEffect({
          kind: 'trn_capacity', delta, expires: 'round', round: this.state.round,
