...
class GameState {
  ...
  restore(snapshot) {
    ...
    for (const code of Object.keys(this.state.sectors)) {
      if (!this.state.broadcast.rows[code]) this.state.broadcast.rows[code] = { power: null, water: null, med: null, parts: null, round: null, report_round: null };
      ...
      // A run saved while COMM could claim a condition carries a word nothing
      // reads any more (2026-09-25). It goes, so no screen can resurrect it.
      delete this.state.broadcast.rows[code].status;
    }
  ...
  emptyBroadcast() {
    ...
    for (const code of Object.keys(this.state ? this.state.sectors : {})) {
      rows[code] = { power: null, water: null, med: null, parts: null, round: null, report_round: null };
    }
