...
function statusWord(cfg, { integrity, status }) {
  ...
  const degradedBelow = Number((cfg || {}).degraded_below ?? 70);
...
function productionFor(game, sector) {
  ...
  let mult = 1;
  if (sector.status === 'BROWNOUT' && !(game.preserving && game.preserving(sector.code, 'PRESERVE_PRODUCTION'))) {
  ...
  // The installed generator level sets the base; every modifier above still
  // applies to it. A sector with no generator keeps its configured production.
  const base = (game.generatorProduction && game.generatorProduction(sector)) || sector.production;
 ...
 */
function processCycle(game, { round = game.state.round } = {}) {
  // THE OPERATING CYCLE (2026-09-21). Not a round and never a pause: it comes
  // round on its own clock while the city keeps running, and it is the only
  // beat that produces, charges upkeep and hands periodic allowances back.
  const cfg = game.cfg;
  const number = game.state.cycle.number;
  const period = game.periodKey ? game.periodKey() : round;
  const summary = { cycle: number, period, round, t: new Date().toISOString(), sectors: {}, missed_upkeep_count: 0 };
  const auto = !!cfg.auto_economy;
  // Since 2026-09-18 a producing sector generates its output by hand, once a
  // round (generateOutput); the pass only reports what it generated. The old
  // automatic production remains behind a scenario switch.
  const manual = cfg.round_output_manual !== false;
  ...
  for (const sector of Object.values(game.state.sectors)) {
    ...
    // 1. production — what the sector generated this cycle, or automatic on the legacy switch
    const generated = sector.round_output && sector.round_output.round === period ? { ...sector.round_output.added } : null;
    const produced = manual ? (generated || {}) : productionFor(game, sector);
    line.produced = produced;
    if (auto && !manual) for (const [k, v] of Object.entries(produced)) sector.inventory[k] = (sector.inventory[k] || 0) + v;
    if (manual && !generated && Object.keys(sector.production || {}).length) line.notes.push('OUTPUT NOT GENERATED');
  ...
  // Every periodic allowance comes back here — Transport's approvals, Medical's
  // heals, AGR's hand, a finished generator upgrade, a released restart crew —
  // and the number turns over first, so a hand dealt now belongs to the cycle
  // that is starting, not the one just ended.
  if (game.cycleRefresh) game.cycleRefresh({ by: 'operating_cycle' });

  game.ticker('cycle', `OPERATING CYCLE ${number} COMPLETE — UPKEEP CHARGED`);
  game.log.write('cycle_processed', { cycle: number, round, summary });
  game.sting('cycle');
  return summary;
}
