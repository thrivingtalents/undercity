...
function processCycle(game, { round = game.state.round } = {}) {
  ...
  const summary = { cycle: number, round, t: new Date().toISOString(), sectors: {}, missed_upkeep_count: 0 };
  ...
  for (const sector of Object.values(game.state.sectors)) {
    ...
    // 1. production — what the sector generated this round, or automatic on the legacy switch
    const generated = sector.round_output && sector.round_output.round === round ? { ...sector.round_output.added } : null;
  ...
  // The CYCLE stamp count only. `stamps_this_round` is deliberately untouched:
  // with the round basis, a cycle boundary must not hand Transport more stamps.
  ...
  game.ticker('cycle', `${round} UPKEEP PROCESSED`);
