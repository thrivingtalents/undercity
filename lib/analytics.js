...
function emptyRound(id) {
  return {
    ...
    rewards: { applied: 0, assigned: 0, duplicates_blocked: 0, force_skipped: 0, overrides: 0, rvu: 0, health_points: 0, resources: {}, archetypes: {}, profiles: {}, by_rvu: {}, fallbacks: 0, resource_units_generated: 0, released_units: 0, assignments: [], list: [] },
...
function analyse(text, { runId = null } = {}) {
  ...
  for (const e of entries) {
    ...
    switch (e.ev) {
      ...
      // -- fault rewards ---------------------------------------------------------
      case 'fault_reward_budget_fallback': R.rewards.fallbacks += 1; break;
      // v17.3: every assignment with its balance inputs, for re-weighting after a playtest.
      case 'fault_reward_assigned': {
        R.rewards.assigned += 1;
        if (e.selected_by === 'fallback') R.rewards.fallbacks += 1;
        if (e.case_profile) R.rewards.profiles[e.case_profile] = (R.rewards.profiles[e.case_profile] || 0) + 1;
        if (e.rvu != null) R.rewards.by_rvu[e.rvu] = (R.rewards.by_rvu[e.rvu] || 0) + 1;
        R.rewards.assignments.push({ t: e.t, fault: e.fault, sector: e.sector, difficulty: e.difficulty_score, target: e.reward_target_rvu, band: e.allowed_reward_rvu, profile: e.case_profile, selected: e.selected, rvu: e.rvu, reserved: e.resource_units_reserved, eligible: (e.eligible_after_filtering || []).length, exact: e.exact_reward_payload ? e.exact_reward_payload.display_label : null });
        break;
      }
      case 'fault_reward_reservation_released': R.rewards.released_units += Number(e.units) || 0; break;
      ...
      case 'fault_reward_applied': {
        ...
        R.rewards.health_points += e.health_points != null ? Number(e.health_points) : (Number(e.health_after) - Number(e.health_before) || 0);
        if (e.archetype) R.rewards.archetypes[e.archetype] = (R.rewards.archetypes[e.archetype] || 0) + 1;
        R.rewards.resource_units_generated += Number(e.resource_units_generated) || 0;
