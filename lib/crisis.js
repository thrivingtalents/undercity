...
function armTimeline(roundId) {
  ...
  // moment its phase was armed, in game time. MASTER TIME runs the whole shift
...
function roundElapsed() {
  const from = Number(this.state.timeline_armed_game_s);
  return Math.max(0, this.state.game_clock_s - (Number.isFinite(from) ? from : 0));
}
